...
def tool_send_telegram(text):
    # Fixed destination from env; agent never chooses a chat id.
    token = os.environ.get("TELEGRAM_BOT_TOKEN", "")
    chat_id = os.environ.get("TELEGRAM_CHAT_ID", "")
    if not token or not chat_id:
        print(ts() + "  [tool call] send_telegram: MISSING TELEGRAM_BOT_TOKEN/TELEGRAM_CHAT_ID")
        return "Error: TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID must be set in the agent environment."
    ...
    if r.status_code != 200:
        print(ts() + "  [tool call] send_telegram: HTTP " + str(r.status_code) + " | " + preview)
        return "Error: Telegram HTTP " + str(r.status_code) + ": " + r.text[:300]
    try:
        data = r.json()
    except ValueError:
        data = {}
    if not data.get("ok"):
        print(ts() + "  [tool call] send_telegram: API not ok | " + preview)
        return "Error: Telegram API: " + r.text[:300]
    print(ts() + "  [tool call] send_telegram: ok chars=" + str(len(body)) + " | " + preview)
    return "sent " + str(len(body)) + " chars"
...
def _dispatch_tool_inner(mcp, name, arguments):
    ...
    if name == "send_telegram":
        return tool_send_telegram(arguments.get("text"))
    ...
    tools.append(
        {
            ...
            "function": {
                "name": "send_telegram",
                "strict": True,
                "description": "Send a short text message to user via Telegram chat. Destination is locked by the harness; do not include a chat id.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "text": {"type": "string", "description": "Message body, max ~4096 chars."},
                    },
                    "required": ["text"],
                    "additionalProperties": False,
                },
...
def main():
    # startup: wipe task_report only. No previous_sessions archiving (life-bot notes
    # would flood it; durable stuff lives as ordinary files under the project dir).
